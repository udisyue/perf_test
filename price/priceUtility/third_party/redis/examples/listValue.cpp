...
struct MiniPriceUnit {
    ...
    MiniPriceUnit() :
            id(0), cost_price(0), price(0), total_price(0), timestamp(0) {
    }
    uint16_t id :8;           // 供应商ID, 0标识该供应商失效(或不存在)
    uint32_t cost_price :24;  // 原始底价
...
std::string ParseFromString(std::vector<char> &bytes) {
    ...
    if (ota_num > 0) {
        ...
        for (size_t i = 0; i < ota_num; ++i) {
            ...
            a_ota["id"] = Json::Value(mpu.ota_id());
            a_ota["cashpay"] = Json::Value(mpu.cash_pay());
            a_ota["mobile"] = Json::Value(mpu.mobile_only());
            a_ota["room_price"] = Json::Value(mpu.price);
            a_ota["total_price"] = Json::Value(mpu.total_price);
            a_ota["cost_price"] = Json::Value(mpu.cost_price);
            a_ota["timestamp"] = Json::Value(mpu.timestamp);
            time_t t = mpu.timestamp;
            char timeformat[64];
            memset(timeformat, 0, 64);
            strftime(timeformat, sizeof(timeformat), "%Y-%m-%d %H:%M:%S", localtime(&t));
            a_ota["timeformat"] = Json::Value(timeformat);
